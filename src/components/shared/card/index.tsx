...
const Card: FC<any> = ({ cardData, onChange, isDefault }) => {
  const cardConfig = content?.cardDetails?.cardType?.find(
    (card) => card?.type === cardData?.card?.brand,
  );
  ...
  return (
    ...
    >
      ...
      <div className="min-w-[37px] min-h-[26px] max-w-[37px] max-h-[26px] flex items-center">
        <Image
          src={cardConfig?.svg || "/assets/svg/mastercard-card.svg"}
          alt="mastercard"
          width={37}
          height={26}
        />
        ...
        <div className="flex flex-col gap-1">
          ...
          <div>
            <Typography as="p" size="md" className="text-white !font-semibold">
              {cardData?.billing_details?.name || "xxxx xxxx"}
            </Typography>
          </div>
        </div>
        <div className="flex flex-col gap-1">
