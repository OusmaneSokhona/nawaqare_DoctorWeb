...
const EditDcotor = () => {
  ...
  const doctor = allDoctors?.RowsData?.find(
    (item: AllDoctorsType) => item?.id?.toString() === id,
  );

  if (!doctor) return <p>No data found</p>;
  const {
  ...
  const [selected, setSelected] = useState<string[]>(doctor?.languages);
    ...
    <div className=" px-7 py-5 max-sm:px-4 max-sm:py-4 mx-auto flex flex-col gap-5">
      <div className="mx-auto relative flex items-center gap-4 h-[200px] w-[250px]">
        ...
        >
          ...
          <div className="flex justify-center font-bold items-center gap-1 bg-white absolute bottom-1 right-1 h-10 w-10 rounded-lg ">
            <Icon icon="fa-solid:edit" width="20" height="20" />
          </div>
        </label>
